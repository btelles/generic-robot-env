...
def extract_config_from_xml(xml_path: Path, robot_name: str) -> RobotConfig:
    ...
    ee_candidates = ["attachment_site", "pinch", "ee", "end_effector", "tool_center", "tcp"]
    ee_site_name = None
    ...
    # If not found, use the last site? Or fail? Let's default to the last site on the last body if unsure, 
    # but for now let's raise/warn if not found or pick the first distinct candidate.
    ...
    if ee_site_name is None:
        # Fallback to the very last site defined, often the EE in standard mjcf
        if model.nsite > 0:
             ee_site_name = mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_SITE, model.nsite - 1)
    ...
    if model.nkey > 0:
        ...
        for i in range(model.nkey):
            # Keyframe names are not directly exposed in python bindings easily as strings in older versions, 
            # but let's check if we can access it.
            # mujoco 3.x bindings: model.key(i).name is not standard. key_name is missing.
            ...
            if key_name == "home":
                ...
                # This is tricky because key_qpos includes ALL joints (including free joints, etc)
                # We will extract it in the env init where we have the joint ids.
                # For now, store the raw key_qpos or just the fact we have a home key.
                # But here we want the numpy array of shape (n_joints,).
                pass
...
class GenericRobotEnv(MujocoGymEnv[Dict[str, Any], np.ndarray]):
    ...
    _rob_gripper_actuator_id: Optional[int]
    ...
    def __init__(
        ...
        camera_id: Optional[int] = None, # Override default camera
    ):
        super().__init__(
        ...
        self._rob_joint_ids = np.array([self._model.joint(name).id for name in robot_config.joint_names], dtype=np.int32)
        self._rob_actuator_ids = np.array([self._model.actuator(name).id for name in robot_config.actuator_names], dtype=np.int32)
        ...
        try:
             ...
             if self._model.nsite > 0:
                  ...
                  self._rob_ee_site_id = self._model.nbody - 1 # Simple fallback to last body center
        
        # Resolve qpos and dof indices for data access
        # qpos indices (for position)
        self._rob_qpos_indices = np.array([self._model.jnt_qposadr[i] for i in self._rob_joint_ids], dtype=np.int32)
        # dof indices (for velocity and torque)
        self._rob_dof_indices = np.array([self._model.jnt_dofadr[i] for i in self._rob_joint_ids], dtype=np.int32)
        
        self._rob_gripper_actuator_id = None
        if robot_config.gripper_actuator_name:
            self._rob_gripper_actuator_id = self._model.actuator(robot_config.gripper_actuator_name).id
        ...
        if robot_config.camera_names:
            for cam_name in robot_config.camera_names:
                self.camera_ids.append(mujoco.mj_name2id(self._model, mujoco.mjtObj.mjOBJ_CAMERA, cam_name))
    ...
    def _setup_observation_space(self):
        ...
        agent_pos_space = {
            "joint_pos": spaces.Box(low=-np.inf, high=np.inf, shape=(n_joints,), dtype=np.float32),
            "joint_vel": spaces.Box(low=-np.inf, high=np.inf, shape=(n_joints,), dtype=np.float32),
            "tcp_pose": spaces.Box(low=-np.inf, high=np.inf, shape=(7,), dtype=np.float32), # Pos + Quat (ee_pose)
            "tcp_vel": spaces.Box(low=-np.inf, high=np.inf, shape=(6,), dtype=np.float32), # Linear + Angular vel
        }
        
        if self._rob_gripper_actuator_id is not None:
            agent_pos_space["gripper_pose"] = spaces.Box(low=-1.0, high=1.0, shape=(1,), dtype=np.float32)
    ...
    def _step_osc(self, action: np.ndarray) -> None:
        ...
        if self._model.nmocap > 0:
            ...
            target_quat = self._data.mocap_quat[0] # Assume Orientation fixed or update it if needed
        else:
            # Use internal state
        ...
        if self._rob_gripper_actuator_id is not None:
             ...
             if ctrl_range[1] > ctrl_range[0]:
                 ...
                 val = (gripper_action + 1) / 2 * (ctrl_range[1] - ctrl_range[0]) + ctrl_range[0]
        ...
        for _ in range(self._n_substeps):
            tau = opspace(
                ...
                pos=target_pos,
                ori=target_quat,
                joint=self._rob_home_position, # Nullspace target. Note: This assumes home_position aligns with qpos of controlled joints
                gravity_comp=True,
            )
    ...
    def _step_joint(self, action: np.ndarray) -> None:
        ...
        # FrankaGymEnv does manual substep loop only for OSC because OSC computes tau every step.
    ...
    def _get_obs(self) -> Dict[str, Any]:
        ...
        # site_xvelp and site_xvelr are not directly exposed in older mujoco bindings easily as separate arrays?
        # Actually in mjData.site_xvelp yes.
        # But we need to check if they are computed. Usually yes if MjModel.opt.enableflags is set or forward called.
        ...
        # Let's compute via Jacobian to be safe and consistent with opspace code if needed, 
        # or rely on data. 
        ...
        if self._rob_gripper_actuator_id is not None:
            ...
            agent_pos["gripper_pose"] = np.array([self._data.ctrl[self._rob_gripper_actuator_id]], dtype=np.float32)
